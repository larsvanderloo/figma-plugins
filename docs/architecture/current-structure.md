...
- `src/sandbox/main.ts` and `src/sandbox/editors/**`: Figma sandbox code. This side may use
...
- `src/shared/types.ts`: typed message-bus contract between the two runtimes.
...
- New Figma mutations: add a focused helper under `src/sandbox/editors/<domain>/` and
...
- New UI controls: prefer a domain editor under `src/ui/components/editors/`
  ...
  bridge state.
- New shared bridge shapes: add to `src/shared/types.ts`, then update both sender and
...
- `src/sandbox/main.ts` should become a thin bootstrap around focused sandbox modules.
- `src/ui/App.vue` should keep shrinking into shell components and composables.
- `src/ui/components/editors/TableEditor.vue` and
  `src/sandbox/editors/table/renderer.ts` are the largest domain modules and should be
  split before adding more table behavior.
- `src/shared/types.ts` and `src/shared/constants.ts` should be split by domain once the
