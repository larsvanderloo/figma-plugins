...
- **Store id** is the camelCase name without `use` prefix: `pluginView`, `chartEditor`. Used by Pinia devtools.
...
- **Scope.** These stores hold state that lives only in the iframe. State that needs to round-trip to the sandbox goes through the message-bus contract in `plugin-src/types.ts`, not through the stores.

## When to add a third store

When two or more components share state that doesn't naturally fit into either `usePluginView` (slide-level view state) or `useChartStore` (chart-editor scratch state). Otherwise, prefer a local `ref()` in the component or a composable in `composables/`.
