...
import TableEditor from '../../../ui/components/TableEditor.vue';
import type {
  TableWrapModel,
  TableRowModel,
  TableCellModel,
...
describe('TableEditor — rendering', () => {
  ...
  it('renders the column header checkbox', () => {
    render(TableEditor, { props: { tableData: makeModel() } });
    const chk = screen.getByRole('switch', { name: /first row is a header/i });
...
describe('TableEditor — column header switch', () => {
  it('emits update:hasColumnHeader with true when checkbox is checked', async () => {
    ...
    const chk = screen.getByRole('switch', { name: /first row is a header/i });
    // USwitch is a button that toggles on click
  ...
  it('emits update:hasColumnHeader with false when switch is toggled from true', async () => {
    const { emitted } = render(TableEditor, {
...
describe('TableEditor — disabled state', () => {
  ...
  it('the column header checkbox is disabled when disabled=true', () => {
    ...
    const chk = screen.getByRole('switch') as HTMLButtonElement;
    // USwitch renders as a button; disabled is via aria-disabled or data-disabled
    expect(
      chk.disabled ||
        chk.getAttribute('aria-disabled') === 'true' ||
        chk.getAttribute('data-disabled') !== null,
    ).toBe(true);
  });
