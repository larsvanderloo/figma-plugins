...
// They are mocked via vi.mock on relative paths so CardEditor's wiring logic
// is tested in isolation without coupling to child component internals.

import { describe, it, expect, vi } from 'vitest';
import { render, screen, fireEvent } from '@testing-library/vue';
import axe from 'axe-core';
import CardEditor from '../../../ui/components/CardEditor.vue';
import type { CardItem } from '../../../ui/components/types.js';

// Stub TitleDescriptionEditor — class="tde-stub" lets tests query by CSS selector
vi.mock('../../../ui/components/TitleDescriptionEditor.vue', () => ({
  default: {
    ...
    emits: ['update:model'],
    template: `
      <fieldset class="tde-stub" :disabled="disabled" data-testid="title-description-editor">
...
// Stub IconPicker — uses modelValue as aria-label; wraps in role="listbox" for axe compliance
      ...
      <div data-testid="icon-picker">
        <div role="listbox" aria-label="Icon options" :aria-disabled="disabled ? 'true' : undefined">
          <button type="button" role="option"
            :aria-label="modelValue || 'circle'"
            :aria-selected="true"
            :disabled="disabled"
            @click="!disabled && $emit('update:modelValue', modelValue)">{{ modelValue }}</button>
        </div>
      </div>
    `,
  },
...
// Stub ImageEditor — shows Replace/Upload based on modelValue.hasImage
// Note: Vue templates cannot use 'new' — emit the bytes from setup() instead.
vi.mock('../../../ui/components/ImageEditor.vue', () => ({
  default: {
    name: 'ImageEditor',
    props: {
      modelValue: Object,
    ...
    setup(_props: unknown, { emit }: { emit: (event: string, ...args: unknown[]) => void }) {
      ...
      return { fakeBytes, emit };
    },
    template: `
      <div data-testid="image-editor">
        <button type="button"
          :aria-label="modelValue && modelValue.hasImage ? 'Replace image' : 'Upload image'"
          :disabled="disabled"
          @click="!disabled && $emit('upload', fakeBytes)">
          {{ modelValue && modelValue.hasImage ? 'Replace' : 'Upload' }}
        </button>
      </div>
    `,
  },
